...
minInst = 0
maxInst = 5
eachFiNo = 100 # fi number for each image input
projectRoot = os.path.dirname(os.path.realpath(__file__))
##############################################


for y in range(minInst, maxInst+1): 
	os.system("cp -r alexnet_img_float-base alexnet_img_float_" + `fiLayerNo` + "_" + `y`)
	os.chdir("alexnet_img_float_" + `fiLayerNo` + "_" + `y`)
